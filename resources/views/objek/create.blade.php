...
@section('header', 'Tambah Objek Baru')
...
<div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6 max-w-2xl mx-auto">
    ...
    <form action="{{ route('objek.store') }}" method="POST">
        @csrf
        <div class="space-y-4">
            <div>
                <label for="nama" class="block text-sm font-medium text-gray-700">Nama Objek / Penduduk</label>
                <input type="text" name="nama" id="nama" value="{{ old('nama') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2" required placeholder="Masukkan nama...">
            </div>
        </div>
