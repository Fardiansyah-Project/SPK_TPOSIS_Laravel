        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody>
                    @forelse($alternatifs as $index => $alt)
                        <tr class="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td class="p-3 text-sm text-gray-800 font-medium">
                                {{ $alt->objek->nama ?? 'Objek telah dihapus' }}</td>
                            <td class="p-3 text-sm text-center">
                                <div class="flex justify-center space-x-2">
                                    <a href="{{ route('alternatif.edit', $alt->id) }}"
                                        class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                    <form action="{{ route('alternatif.destroy', $alt->id) }}" method="POST"
                                        onsubmit="return confirm('Yakin ingin menghapus alternatif ini beserta penilaiannya?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="p-6 text-center text-gray-500">Belum ada data kandidat alternatif.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
