...
<head>
    ...
    <title>@yield('title', 'Sistem Pendukung Keputusan BPNT')</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <style>
        body { font-family: 'Inter', sans-serif; background-color: #f8fafc; }
        .sidebar-link { display: flex; align-items: center; padding: 0.75rem 1rem; color: #cbd5e1; border-radius: 0.5rem; transition: all 0.2s; }
...
<body class="flex h-screen overflow-hidden text-gray-800">
    ...
    <aside class="w-64 bg-slate-900 flex-shrink-0 hidden md:flex flex-col">
        <div class="h-16 flex items-center justify-center bg-slate-950">
            <span class="text-white font-bold text-xl tracking-wider">SPK TOPSIS</span>
        </div>
        <div class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
            <div class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 px-3 mt-4">Menu Utama</div>
            <a href="{{ route('topsis.index') }}" class="sidebar-link {{ request()->routeIs('topsis.*') ? 'active' : '' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
                Hasil TOPSIS
            </a>
            
            <div class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 px-3 mt-6">Data Master</div>
            <a href="{{ route('kriteria.index') }}" class="sidebar-link {{ request()->routeIs('kriteria.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('sub-kriteria.index') }}" class="sidebar-link {{ request()->routeIs('sub-kriteria.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('objek.index') }}" class="sidebar-link {{ request()->routeIs('objek.*') ? 'active' : '' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                Objek Penduduk
            </a>
            <a href="{{ route('alternatif.index') }}" class="sidebar-link {{ request()->routeIs('alternatif.*') ? 'active' : '' }}">
