...
@section('header', 'Tambah Sub Kriteria Baru')
    ...
    <form action="{{ route('sub-kriteria.store') }}" method="POST">
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nama" id="nama" value="{{ old('nama') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2" required placeholder="Contoh: < 1 Juta">
            </div>

            <div>
                <label for="nilai" class="block text-sm font-medium text-gray-700">Nilai (Angka)</label>
                <input type="number" step="0.01" name="nilai" id="nilai" value="{{ old('nilai') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2" required placeholder="Contoh: 1, 2, 3..">
            </div>
        </div>
